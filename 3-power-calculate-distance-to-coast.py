...
dist = dist.where(dist.primary_fuel.isin(['Biomass','Coal','Cogeneration','Gas','Nuclear','Oil','Other','Petcoke','Waste']), drop=True)
...
def get_state(x):
    ...
    if not found.empty:
        return found['facility_state'].values[0]
    else:
        return 'unknown'

dist['facility_state'] = dist.to_dataframe().apply(lambda x: get_state(x), axis=1)
